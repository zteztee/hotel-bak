<template>
    <div>
  <!-- Form -->
  <el-dialog title="团体房间入住" :visible="clickz" @close="back" :modal=false>
            <el-row :gutter="20">
                <div>房间号:</div>
                <el-col :span="4" v-for="(item,index) in checkbox" :key="index" :xs="12" :sm="6" :md="4" :label="item">
                    {{ item }}   
                </el-col>
            </el-row>
  ...
  import { Message } from 'element-ui'
    ...
    export default {
      ...
      methods:{
        ...
        getCount(){
            this.ruleForm.payneed=this.ruleForm.days*this.countmoney
        },
        back(){
      ...
      addClients(){
        this.ruleForm.roomlist=this.checkbox
